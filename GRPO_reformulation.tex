...
\label{sec:GRPO}
    ...
    For policies conditioned on context $s$,
    \[
    ...
    This mirrors the approximation already employed in PPO, where the KL term is used as a trust-region heuristic rather than as an unbiased estimator under the target policy measure. In both cases, stability arises from conservative updates rather than from exact optimization of a KL-constrained objective.
    ...
    The method does not fail; rather, the KL terminology obscures the deeper point: the written objective is a proxy for stability, not a literal instantiation of a KL-constrained optimization problem.
